...
class external extends external_api {
    ...
    public static function generate_time_report($jsonformdata) {
        ...
        $file = $fs->get_file($serialiseddata['contextid'], 
            ...
            '0', 
            '/', 
            $timereport->generate_file_name($serialiseddata['userid'], $serialiseddata['start'], $serialiseddata['end'])
        );
        if ($file) {
    ...
    public static function poll_report_file($jsonformdata) {
        ...
        if ($user) {
            $fs = get_file_storage();
            $filename = $timereport->generate_file_name($userid, $serialiseddata['start'], $serialiseddata['end']);
